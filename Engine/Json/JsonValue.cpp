...
void JsonValue::CopyAssetRefValue(AssetRef* out, AssetType assetType)
{
	if (this->type == JsonValueTypeString)
	{
		StringUtil::CopyString(out->filePath, this->stringValue, AssetMaxFilePathLength);
...
void JsonValue::CopySphereValue(Sphere* out)
{
	if (this->type == JsonValueTypeObject)
	{
		IJsonValue* position = this->GetObjectProperty("position");
		if (position != null)
		{
			position->CopyVec3Value(&out->position);
		}

		IJsonValue* radius = this->GetObjectProperty("radius");
		if (radius != null)
		{
			out->radius = radius->GetFloatValue();
		}
	}
}
...
void JsonValue::CopyRbgFloatValue(RgbFloat* out)
{
	if (this->type == JsonValueTypeArray && this->arrayElements.GetLength() == 3)
	{
